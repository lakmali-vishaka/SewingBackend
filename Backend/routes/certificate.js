...
const SewingItem = require("../models/Certificates");
...
router.post("/add", upload.single('image'), async (req, res) => {
  try {
    ...
    const newItem = new Certificates({ imageURL });
    await newItem.save();

    res.status(201).json({ message: "certificates added", newItem });
  } catch (err) {
    res.status(500).json({ error: "Failed to add cerficates", err });
  }
});
...
router.delete("/delete", async (req, res) => {
  try {
    ...
    const itemToDelete = await Certificates.findOne({ imageURL: image });
    if (!itemToDelete) {
    ...
    res.status(500).json({ error: "Failed to delete certificate", err });
  }
});
